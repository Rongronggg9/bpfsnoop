...
emit_bpfsnoop_kmulti_event(struct pt_regs *ctx)
{
    ...
    can_output_lbr = !cfg->both_entry_exit || is_entry;
    ...
    return output_bpfsnoop_event(ctx, event_type, session_id, bpf_get_func_ip(ctx),
...
SEC("kprobe.session")
int BPF_KPROBE(bpfsnoop_ksession)
{
    return emit_bpfsnoop_kmulti_event(ctx);
}
